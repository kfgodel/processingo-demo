...
public class DemoBouncingBall {
  ...
  public static void main(String[] args) {
    ...
    WorkerThread.create()
      .execute(WorkerTask.periodicWith(TimeQuantity.of(150, TimeUnit.MILLISECONDS), ball::move));
      ...
      .create(ViewportDefinition.window(ViewSize.of(640, 480), ProcessingRenderer.P2D),
        (canvas)-> {
          BackgroundVisual.create(0xAAAAAAAA)
            .applyOn(canvas);
          EllipseVisual.create(ball.position().componentProduct(vector(640, 480)), vector(ball.diameter().multiply(640), ball.diameter().multiply(480)))
            .applyOn(canvas);
        });
